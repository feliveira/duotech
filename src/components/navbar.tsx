...
export default function Navbar( ) {
    ...
    return (
        <div className="fixed bottom-0 md:inset-y-0 md:space-y-4 py-2 flex md:flex-col w-full md:w-fit border-t border-t[#E5E5E5] md:border-t-0 md:border-r md:border-r-neutral-200 md:h-full bg-whiteitems-center md:items-start justify-center">
                    ...
                    {routes.map(route => (
                        <Link href={route.href} key={route.href} className={cn("flex p-3 md:w-full justify-center md:justify-start rounded-xl transition-all delay-100", pathName === route.href ? "border-[3px] border-darkBlue bg-lightBlue text-darkBlue" : "hover:bg-neutral-200 text-gray-600" )} >
                            <div className="flex items-center md:flex-1 w-fit md:w-40">
                                ...
                                <p className="hidden md:flex text-sm md:text-base uppercase font-medium">{route.label}</p>
                            </div>
                        </Link>
                    ))}
                </div>
